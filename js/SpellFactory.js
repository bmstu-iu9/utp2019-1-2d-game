...
class SpellFactory {
    static CreateFireBall(x, y, vector) {
        let hitbox = undefined//new Hitbox('CircleHitbox', new Vector2d(x, y), 16)
        ...
        result.onCollide = (collision) =>{
            if(collision.obstacle instanceof AABB){
                result.hitbox.correctPosition(collision);
                result.actor.changePosition(collision.distance);
                result.Update = () => {
                    result.actor.update()
