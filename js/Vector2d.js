...
class Vector2d{
    ...
    //Очевидно, что ,передавая вектор-"контейнер", вы не изеняете вектор,
    ...
    //который и будет возвращен как результат.
    mul(number,res){
        if (res===undefined){
            this.x*=number;
            this.y*=number;
    ...
    // Метод set(...) работает очевидным образом. Он может принимать либо вектор,либо -- два числа.
    set(vector2d_or_x,y){
        if (y===undefined){
            this.x=vector2d_or_x.x;
            this.y=vector2d_or_x.y;
